...
/**
 * Provides a representation of the layers in the graph. A layer is a set of homogeneous nodes having the same
 * distance from the sources (eg: the initial column of sources or all the samples after the source column).
 * <p/>
 * Basically we allow to represent the structure &lt;layer no., row, column (in the header)&gt; =&gt; value
 * <p/>
 * <dl><dt>date</dt><dd>May 5, 2010</dd></dl>
 *
 * @author brandizi
 */
class LayerContents {
    ...
    /**
     * The contents of a single layer, which is like a table. For efficiency, we store the table by using
     * a coordinate system, ie a double map like [col,row] =&gt; value. Moreover, we maintain the headers
     * separately and some cache information (no of rows in each colum).
     */
    private static class LayerContent {
        ...
        final SortedObjectStore<Integer, Integer, String> table =
                new SortedObjectStore<Integer, Integer, String>();
        /**
         * we maintain the number of rows in each column, which is computed from the max column index set
...
//	}
    ...
    /**
     * The value for a given cell in a certain layer. icol must be &lt {@link #headersSize(int)}, irow can be any
     * positive integer, nulls will be returned in case out-of-bound coordinates are passed
     */
...
//	}
    ...
    /**
     * The max number of rows in all the contents. You should see the final structure as a table having the same number
