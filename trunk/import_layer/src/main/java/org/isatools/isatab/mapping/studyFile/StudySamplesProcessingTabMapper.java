...
/**
 * Maps the processing steps about sample extraction from source
 * <p/>
 * Dec 2007
 *
 * @author brandizi
 */
public class StudySamplesProcessingTabMapper extends ProcessingsTabMapper {

    public StudySamplesProcessingTabMapper(BIIObjectStore store, SectionInstance sectionInstance) {
        super(store, sectionInstance);
        nodeMappersConfig.put("Source Name", SourceTabMapper.class);
    ...
    /**
     * Does some pre-processing, such as setting {@link AssayGroup} properties.
     */
    @Override
    public BIIObjectStore map() {
        ...
        // Store this samples instance for all the assays about this study.
        ...
        if (study != null) {
            for (AssayGroup assayGroup : store.valuesOfType(AssayGroup.class)) {
                if (assayGroup.getStudy() == study) {
                    assayGroup.setSampleSectionInstance(sectionInstance);
                }
            }
        }
