 ...
 * <p/>
 * TODO: The raw data file is actually an attribute of the MS/RUN, so we should be able to associate a data node too to
 ...
 * @author brandizi
 */
public class MsDataTabMapper extends DataNodeTabMapper {

    @SuppressWarnings("unchecked")
    public MsDataTabMapper(BIIObjectStore store, SectionInstance sectionInstance, int fieldIndex, int endField) {
        super(store, sectionInstance, fieldIndex, endField);
        // "Raw Spectral Data File" is the first col.
    }

    @Override
    public Data map(int recordIndex) {
        Data data = super.map(recordIndex);
        data.setUrl(data.getName());
    ...
    public Data newMappedObject() throws InstantiationException, IllegalAccessException {
        ...
        //
        return new Data(
                "", new DataType("bii:ms_spec_raw_data", "MS/SPEC Raw Data", new ReferenceSource("bii:data_types", "bii:data_types"))
        );
    }
