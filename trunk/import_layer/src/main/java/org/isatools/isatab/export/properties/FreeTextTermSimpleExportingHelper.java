...
/**
 * Free text exporting helper that works to the case of the same field used as either label or ontology entry
 * label.
 * <p/>
 ...
 */
public class FreeTextTermSimpleExportingHelper<ST extends Identifiable, FT extends FreeTextTerm>
        extends PropertyExportingHelper<ST> {
    private FreeTextSimpleExportingUtility termXutil;


    public FreeTextTermSimpleExportingHelper(BIIObjectStore store, String propertyName,
                                             String fieldName, String accessionFieldName, String sourceFieldName
    ) {
        super(store, propertyName);
        this.termXutil = new FreeTextSimpleExportingUtility(store, fieldName, accessionFieldName, sourceFieldName);
    }

    public FreeTextTermSimpleExportingHelper(BIIObjectStore store, String propertyName,
                                             String fieldName
    ) {
        this(store, propertyName, fieldName, null, null);
    }


    @Override
    public Record export(ST source, Record record) {
        FT term = exportProperty(source);
        return termXutil.export(term, record);
    }
     ...
     */
    private FT exportProperty(ST source) {
        FT value;
        try {
            value = (FT) PropertyUtils.getProperty(source, getPropertyName());
        }
        catch (Exception e) {
            throw new TabInternalErrorException(i18n.msg(
                    "pb_export_class_property", getPropertyName(), source.getClass().getSimpleName(), e.getMessage()), e
            );
        }
        return value;
    }
