 ...
 * @author brandizi
 */
public class InvestigationContactTabMapper extends ClassTabMapper<Contact> {
    private MappingUtils mappingUtils;

    @SuppressWarnings("unchecked")
    public InvestigationContactTabMapper(BIIObjectStore store, SectionInstance sectionInstance) {
        super(store, sectionInstance);

        mappingHelpersConfig.put("Investigation Person First Name",
                new MappingHelperConfig<StringPropertyMappingHelper>(
                        StringPropertyMappingHelper.class, new String[][]{{"propertyName", "firstName"}}
                )
        );
        mappingHelpersConfig.put("Investigation Person Mid Initials",
                new MappingHelperConfig<StringPropertyMappingHelper>(
                        StringPropertyMappingHelper.class, new String[][]{{"propertyName", "midInitials"}}
                )
        );
        mappingHelpersConfig.put("Investigation Person Last Name",
                new MappingHelperConfig<StringPropertyMappingHelper>(
                        StringPropertyMappingHelper.class, new String[][]{{"propertyName", "lastName"}}
                )
        );
        mappingHelpersConfig.put("Investigation Person Email",
                new MappingHelperConfig<StringPropertyMappingHelper>(
                        StringPropertyMappingHelper.class, new String[][]{{"propertyName", "email"}}
                )
        );
        mappingHelpersConfig.put("Investigation Person Phone",
                new MappingHelperConfig<StringPropertyMappingHelper>(
                        StringPropertyMappingHelper.class, new String[][]{{"propertyName", "phone"}}
                )
        );
        mappingHelpersConfig.put("Investigation Person Fax",
                new MappingHelperConfig<StringPropertyMappingHelper>(
                        StringPropertyMappingHelper.class, new String[][]{{"propertyName", "fax"}}
                )
        );
        mappingHelpersConfig.put("Investigation Person Address",
                new MappingHelperConfig<StringPropertyMappingHelper>(
                        StringPropertyMappingHelper.class, new String[][]{{"propertyName", "address"}}
                )
        );
        mappingHelpersConfig.put("Investigation Person Affiliation",
                new MappingHelperConfig<StringPropertyMappingHelper>(
    ...
    public Contact map(int recordIndex) {
        ...
        if (investigations == null || investigations.size() == 0) {
            throw new TabInternalErrorException("No investigation to link a mapped contact to, contact:" + contact);
        }

        if (investigations.size() > 1) {
            throw new TabInternalErrorException("Too many investigations in the store (" + investigations.size() + ")");
        }

        Investigation investigation = investigations.iterator().next();
        contact.setInvestigation(investigation);
    ...
    /**
     * null, we don't need to save this
     */
    @Override
