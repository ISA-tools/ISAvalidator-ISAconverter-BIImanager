...
public class StudyTabMapper extends ClassTabMapper<Study> {
    ...
    public StudyTabMapper(BIIObjectStore store, SectionInstance sectionInstance) {
        ...
        mappingHelpersConfig.put("Study Submission Date", new MappingHelperConfig<DatePropertyMappingHelper>(
    ...
    public Study map(int recordIndex) {
        ...
        if (StringUtils.trimToNull(mappedStudy.getAcc()) == null) {
            throw new TabMissingValueException(i18n.msg("study_without_accession_titled", mappedStudy.getTitle()));
        }
        ...
        if (investigations == null || investigations.size() == 0) {
            ...
            if (investigations.size() > 1) {
                throw new TabInternalErrorException("Internal Error, Study Mapper, too many investigations in the store (" + investigations.size() + ")");
            }
        ...
        mappedStudy.addAnnotation(new Annotation(new AnnotationType("investigationFile"), sectionInstance.getFileId()));


        log.trace("New/Saved mapped study: " + mappedStudy);
        return mappedStudy;
    }
    ...
    public List<Integer> getMatchedFieldIndexes() {
        ...
        addMatchedField("Study File Name", result);
