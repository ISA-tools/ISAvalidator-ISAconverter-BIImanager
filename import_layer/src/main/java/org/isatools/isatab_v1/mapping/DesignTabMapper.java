 ...
 * @author brandizi
 */
public class DesignTabMapper extends ClassTabMapper<Design> implements StudyComponentTabMapper {
    ...
    public Design map(int recordIndex) {
        ...
        if (mappedStudy == null) {
            throw new TabInternalErrorException(
                    "Design mapper: No study to attach this design to, design:" + design
            );
        } else {
        ...
        // Other stuff
        log.trace("New mapped study design: " + design);
        return design;
    }


    public Design newMappedObject() {
        return new Design();
    }

    /**
     * Returns null, we don't need to store these and we don't know how to generate a key
     */
    @Override
    public String getStoreKey(Design mappedObject) {
        return null;
    }
    ...
    public List<Integer> getMatchedFieldIndexes() {
        ...
        addMatchedField("Study Design Type", result);
        addMatchedField("Study Design Type Term Accession Number", result);
        addMatchedField("Study Design Type Term Source REF", result);
        return result;
    }
