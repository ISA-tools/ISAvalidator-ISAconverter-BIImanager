 ...
 * @author brandizi
 */
public class ContactTabMapper extends ClassTabMapper<Contact> implements StudyComponentTabMapper {
    private MappingUtils mappingUtils;
    private Study mappedStudy;

    @SuppressWarnings("unchecked")
    public ContactTabMapper(BIIObjectStore store, SectionInstance sectionInstance) {
        super(store, sectionInstance);

        mappingHelpersConfig.put("Person First Name",
                new MappingHelperConfig<StringPropertyMappingHelper>(
                        StringPropertyMappingHelper.class, new String[][]{{"propertyName", "firstName"}}
                )
        );
        mappingHelpersConfig.put("Person Last Name",
                new MappingHelperConfig<StringPropertyMappingHelper>(
                        StringPropertyMappingHelper.class, new String[][]{{"propertyName", "lastName"}}
                )
        );
        mappingHelpersConfig.put("Person Email",
                new MappingHelperConfig<StringPropertyMappingHelper>(
    ...
    public Contact map(int recordIndex) {
        ...
        // Basic properties
        Contact rval = super.map(recordIndex);
        ...
        if (mappedStudy == null) {
            throw new TabMissingValueException(i18n.msg("contact_no_study", rval));
        } else {
            rval.setStudy(mappedStudy);
            mappedStudy.addContact(rval);
        }

        log.trace("New/Saved mapped contact: " + rval);
        return rval;
    }

    /**
     * Uses firstname\midInitials\lastname\email
     */
    @Override
    public String getStoreKey(Contact mappedObject) {
        return mappedObject == null ? null
                : StringUtils.trimToEmpty(mappedObject.getFirstName())
                + "\\" + StringUtils.trimToEmpty(mappedObject.getMidInitials())
                + "\\" + StringUtils.trimToEmpty(mappedObject.getLastName()) +
                "\\" + StringUtils.trimToEmpty(mappedObject.getEmail());
    }
