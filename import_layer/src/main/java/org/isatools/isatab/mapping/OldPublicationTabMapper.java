...
/**
 * Maps the publications in the Investigation file. This is the old version, prior to ISATAB 0.2.
 * <p/>
 * Dec 2007
 *
 * @author brandizi
 */
public class OldPublicationTabMapper extends ClassTabMapper<Publication> {
    private MappingUtils mappingUtils;


    @SuppressWarnings("unchecked")
    public OldPublicationTabMapper(BIIObjectStore store, SectionInstance sectionInstance) {
        super(store, sectionInstance);

        mappingHelpersConfig.put("PubMed ID", new MappingHelperConfig<StringPropertyMappingHelper>(
                StringPropertyMappingHelper.class, new String[][]{{"propertyName", "pmid"}}
        ));
        mappingHelpersConfig.put("Publication Author List", new MappingHelperConfig<StringPropertyMappingHelper>(
    ...
    public Publication map(int recordIndex) {
        ...
        PublicationStatus status = (PublicationStatus) mappingUtils.createOntologyEntry(
                getSectionInstance(), recordIndex, "Publication Status Term Accession Number", "Publication Status", source, PublicationStatus.class
        );
    ...
    public String getStoreKey(Publication mappedObject) {
        return mappedObject == null ? null : mappedObject.getPmid();
    }
