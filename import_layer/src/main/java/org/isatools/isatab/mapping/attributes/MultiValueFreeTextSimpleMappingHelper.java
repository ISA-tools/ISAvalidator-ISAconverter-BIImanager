 ...
 * This version use the same field for either the term free-text value or the value of the ontology entry
 * attached to it.
 * <p/>
 ...
 */
public abstract class MultiValueFreeTextSimpleMappingHelper<T extends Identifiable, PT extends FreeTextTerm>
        extends PropertyMappingHelper<T, Collection<PT>> {
    private final String accessionFieldName, sourceFieldName;
    private final MappingUtils mappingUtils;

    public MultiValueFreeTextSimpleMappingHelper(
    ...
    /**
     * @param fieldName          e.g.: "Parameter", this is used as free-text value and additionally as the name
     *                           of the ontology entry, if accession/source are provided.
     * @param accessionFieldName e.g.: "Parameter Type Accession Number"
     * @param sourceFieldName    e.g.: "Parameter Type Source REF"
     * @param propertyName       the property name to be mapped (needed only as a reference), e.g.: "parameters"
     */
    public MultiValueFreeTextSimpleMappingHelper(
    ...
    public Collection<PT> mapProperty(int recordIndex) {
        ...
        if (!(propSs.length == 0 && accSs.length == 0 && srcSs.length == 0
                || propSs.length > 0 && accSs.length == srcSs.length)) {
        ...
        Class<PT> ptClass = ReflectionUtils.getTypeArgument(MultiValueFreeTextSimpleMappingHelper.class, this.getClass(), 1);
        for (int i = 0; i < propSs.length; i++) {
            ...
            PT term = mappingUtils.createTerm(accSi, propSi, srcSi, ptClass, OntologyTerm.class);
            if (term != null) {
                result.add(term);
            }
        }
