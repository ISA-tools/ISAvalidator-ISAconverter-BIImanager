...
import org.isatools.isatab.mapping.attributes.ProtocolComponentMappingHelper;
import org.isatools.isatab.mapping.attributes.ProtocolParameterMappingHelper;
...
public class ProtocolTabMapper extends ClassTabMapper<Protocol> implements StudyComponentTabMapper {
    ...
    public ProtocolTabMapper(BIIObjectStore store, SectionInstance sectionInstance) {
        ...
        mappingHelpersConfig.put("Protocol Name", new MappingHelperConfig<StringPropertyMappingHelper>(
                StringPropertyMappingHelper.class, new String[][]{{"propertyName", "name"}}
        ));
        mappingHelpersConfig.put("Protocol Description", new MappingHelperConfig<StringPropertyMappingHelper>(
                StringPropertyMappingHelper.class, new String[][]{{"propertyName", "description"}}
        ));
        mappingHelpersConfig.put("Protocol URI", new MappingHelperConfig<StringPropertyMappingHelper>(
                StringPropertyMappingHelper.class, new String[][]{{"propertyName", "uri"}}
        ));
        mappingHelpersConfig.put("Protocol Version", new MappingHelperConfig<StringPropertyMappingHelper>(
                StringPropertyMappingHelper.class, new String[][]{{"propertyName", "version"}}
        ));
        mappingHelpersConfig.put("Protocol Parameters", new MappingHelperConfig<ProtocolParameterMappingHelper>(
                ProtocolParameterMappingHelper.class, (String[][]) null
        ));
        mappingHelpersConfig.put("Protocol Components", new MappingHelperConfig<ProtocolComponentMappingHelper>(
    ...
    public Protocol map(int recordIndex) {
        ...
        ProtocolType type = mappingUtils.createOntologyEntry(
                ...
                "Protocol Type Term Accession Number", "Protocol Type", "Protocol Type Term Source REF",
                ProtocolType.class);
        if (type != null) {
            proto.setType(type);
        }
        ...
        //
        if (mappedStudy == null) {
            throw new TabInternalErrorException(i18n.msg("orphan_protocol", proto));
        } else {
    ...
    public String getStoreKey(Protocol mappedObject) {
        ...
        if (mappedStudy == null) {
            throw new TabInternalErrorException(i18n.msg("orphan_protocol", "\"" + mappedObject.getName() + "\""));
        }
