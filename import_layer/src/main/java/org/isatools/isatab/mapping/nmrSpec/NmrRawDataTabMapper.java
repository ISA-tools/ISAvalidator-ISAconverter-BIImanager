...
/**
 * Maps raw Mass Spectrometry data nodes.
 * <p/>
 * TODO: The raw data file is actually an attribute of the NMR/RUN, so we should be able to associate a data node too to
 ...
 * @author brandizi
 */
public class NmrRawDataTabMapper extends DataNodeTabMapper {

    @SuppressWarnings("unchecked")
    public NmrRawDataTabMapper(BIIObjectStore store, SectionInstance sectionInstance, int fieldIndex, int endField) {
        // "Free Induction Decay Data File" is the matching field.
    ...
    public Data newMappedObject() throws InstantiationException, IllegalAccessException {
        ...
        //
        return new Data(
                "", new DataType("bii:nmr_spec_raw_data", "NMR Spectral Raw Data", new ReferenceSource("bii:data_types", "bii:data_types"))
        );
    }
    ...
    protected String getName(int recordIndex, Data mappedObject) {
        SectionInstance sectionInstance = getSectionInstance();
        String dataFileName = StringUtils.trimToEmpty(sectionInstance.getString(recordIndex, getFieldIndex()));
        String assayName = StringUtils.trimToEmpty(sectionInstance.getString(recordIndex, "NMR Assay Name"));
