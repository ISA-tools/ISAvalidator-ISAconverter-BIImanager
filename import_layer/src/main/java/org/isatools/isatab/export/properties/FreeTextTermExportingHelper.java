...
/**
 * Exports a FreeTerm that possibly has OEs attached.
 * <p/>
 * <p><b>date</b>: Aug, 2008</p>
 *
 * @author brandizi
 * @param <ST> the source type which the exported property is taken from
 * @param <FT> the type it is exported
 */
public class FreeTextTermExportingHelper<ST extends Identifiable, FT extends FreeTextTerm>
        extends PropertyExportingHelper<ST> {
    private FreeTextExportingUtility termXutil;


    public FreeTextTermExportingHelper(BIIObjectStore store, String propertyName,
        ...
        this.termXutil = new FreeTextExportingUtility(store,
                fieldName, ontologyEntryFieldName, accessionFieldName, sourceFieldName
        );
    }
    ...
    private FT exportProperty(ST source) {
        ...
        try {
            ...
            throw new TabInternalErrorException(i18n.msg(
                    "pb_export_class_property", getPropertyName(), source.getClass().getSimpleName(), e.getMessage()
            ), e);
        }
        return value;
    }
