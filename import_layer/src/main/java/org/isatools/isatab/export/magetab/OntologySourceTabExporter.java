...
import org.isatools.tablib.export.properties.NullPropertyExportingHelper;
...
/**
 * The Ontology Source exporter for the MAGETAB format.
 * TODO: export only the pertinent sources.
 * <p/>
 * date: Apr 3, 2008
 *
 * @author brandizi
...
public class OntologySourceTabExporter extends ClassTabExporter<ReferenceSource> {
    ...
    public OntologySourceTabExporter(BIIObjectStore store) {
        super(store);
        propertyExportingHelpers = new ArrayList<PropertyExportingHelper<ReferenceSource>>();
        ...
        propertyExportingHelpers.add(new NullPropertyExportingHelper<ReferenceSource>(store, "Term Source Version"));
    ...
    @Override
    public Record export(ReferenceSource source, Record record) {
        if (source != null && "BII:NULL-SOURCE".equals(source.getAcc())) {
            log.trace("Ignoring BII:NULL-SOURCE in MAGETAB export");
            return record;
        }
        return super.export(source, record);
    }


    @Override
    protected Section getSection() {
        return MAGETABExporter.getMAGETABSchema().getDescendant("ontoSources", Section.class);
    }
