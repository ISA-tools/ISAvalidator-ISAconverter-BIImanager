...
public class GUIISATABUnloaderTest extends TransactionalDBUnitEJB3DAOTest {

	public GUIISATABUnloaderTest() throws Exception {
		super();
	}
	...
	public void testUnloading() throws Exception {
		out.println("\n\n_______________________ GUI ISATAB Unloading Test _______________________\n\n");
		...
		String repoPath = baseDir + "/target/bii_test_repo";
		...
		assertTrue("Sigh! meta-data repository directory was not created for study BII-S-1!",
				new File(repoPath + "/meta_data/study_" + study1FileName).exists()
		);
		assertTrue("Sigh! MEDA repository directory was not created for study 5!",
				new File(repoPath + "/meda/study_" + study1FileName).exists()
		);
		assertTrue("Sigh! meta-data file 'a_metabolome.txt' not found in the repository!",
				new File(repoPath + "/meta_data/study_" + study1FileName + "/a_metabolome.txt").exists()
		);
		assertTrue("Sigh! data file 'JIC1_Carbon_0.07_Internal_1_1.txt' not found in the repository!",
				new File(repoPath + "/meda/study_" + study1FileName + "/raw_data/JIC1_Carbon_0.07_Internal_1_1.txt").exists()
		);
		...
		GUIInvokerResult result = unloader.loadStudiesFromDB();
		assertEquals("Ouch! unloader report an error (during loadStudiesFromDB())", GUIInvokerResult.SUCCESS, result);

		result = unloader.unload(unloader.getRetrievedStudies());
		assertEquals("Ouch! unloader report an error", GUIInvokerResult.SUCCESS, result);


		// Check it has gone correctly
		// 
		...
		assertTrue("Sigh! meta-data file 'a_metabolome.txt' still in the repository!",
				!new File(repoPath + "/meda/study_" + study1FileName + "/a_metabolome.txt").exists()
		);
		assertTrue("Sigh! data file 'JIC1_Carbon_0.07_Internal_1_1.txt' still in the repository!",
				!new File(repoPath + "/meda/study_" + study1FileName + "/raw_data/JIC1_Carbon_0.07_Internal_1_1.txt").exists()
		);

		out.println("\n\n___________________ /end: GUI ISATAB Unloading Test ___________________\n\n");
	}
